...
bool BleMouse::startPairingMode(void) {
  ...
  if (this->isConnected()) {
    this->server->disconnect(this->server->getConnId());
  }
...
void BleMouse::taskServer(void* pvParameter) {
  ...
  // Keep pairing simple across desktops/OSes: HID without mandatory bonding.
  ...
  bleMouseInstance->advertising = pAdvertising;
